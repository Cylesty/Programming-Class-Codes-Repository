# Simple search program that searches for keywords or keyphrases in a database
...
# Finds and extracts tags from a file
def find_tag(file_name, tag):
    ...
    with open(file_name, 'r') as f:
        ...
        found_tag = found_tag[0]                        # Gets the first element from the list as a string
...
# Creates a list of data for the output
...
# Looks for keywords in the files
def search_files(search_tag, keyword_1, operator, keyword_2):
    main_list = [[], [], [], [], []]                # Placeholder for list ( 2-dimensional )
...
# Prints out list of data as output
...
if __name__ == "__main__":      # Run code only when it's not being imported
    ...
    path = "C:\\Users\\matri\\Desktop\\Codes\\3_Programming_Tasks\\TP02\\dataset"  # Sets path variable
    ...
    if len(sys.argv) == 3:                                                      # If user input has search_tag and keyword_1 then proceed
        operator = 'NONE'                                                       # Default operator NONE is given, if no operator is provided
        result = search_files(sys.argv[1], sys.argv[2], operator, '')           # Calls 'search_files' based on specified criteria

    elif len(sys.argv) == 5:                                                    # If user input has search_tag, keyword_1, operator, and keyword_2 then proceed
        ...
        print()
        print("The program argument is incorrect.")                             # Displays an error message for inputs not in the correct format
        print()
        sys.exit(1)                                                             # Exits the program

    file, province, class_, subclass_, judicial_inst = result                   # Unpacks the 'result' list into separate variables for easier access to the contents
